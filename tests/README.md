...
├── integration_real/        # Integration tests with real models
│   └── test_skinning_real.py
└── README.md
```
...
### Integration Tests (`integration_real/`)
- Tests with real model loading (marked with `@pytest.mark.real_model`)
- Tests config serialization and override logic
- **Requires full UniRig setup with Blender** for real model tests

```bash
# Run mocked integration tests only
pytest tests/integration_real/ -v -m integration

# Run real model tests (requires setup)
...
# Real model tests (slow, requires Blender)
pytest tests/integration_real/ -v -m real_model
```
...
2. **Integration tests**: Add to `integration_real/` with appropriate markers
3. **New node tests**: Follow the pattern in `TestSkeletonExtractionNode`
